...
st.title("🏠 Virtual Room Furnishing")
...
with st.sidebar:
    ...
    prompt = st.text_area("Prompt",
                          value="A beautifully furnished room with modern furniture, warm lighting, and elegant decor.")

    use_negative_prompt = st.checkbox("Use Negative Prompt", value=True)

    if use_negative_prompt:
        negative_prompt = st.text_area("Negative Prompt", value="ugly, blurry, poor quality, distorted, unrealistic")
    ...
    if st.button("Load Model", type="primary"):
        with st.spinner("Loading model..."):
            ...
            controlnet = ControlNetModel.from_pretrained("lllyasviel/control_v11p_sd15_mlsd", torch_dtype=dtype).to(device)
