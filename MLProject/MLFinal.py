...
with tab5:
    ...
    st.markdown(
        '<div class="info-text">Upload a room image, detect objects, and remove unwanted items with AI inpainting technology.</div>',
        unsafe_allow_html=True)
    ...
    with col1:
        ...
        uploaded_file_remove = st.file_uploader("Upload Furnished Room Image", type=["png", "jpg", "jpeg"],
                                                key="remove_upload")
            ...
            if st.session_state.object_detector_loaded:
                with st.spinner("Detecting objects..."):
                    try:
                        ...
                        st.image(image_with_boxes, caption="Detected Objects - Select to Remove",
    ...
    with col2:
        ...
        if 'detected_objects' in st.session_state and st.session_state['detected_objects']:
            ...
            selected_index = st.selectbox("Select Object to Remove", range(len(object_labels)),
                                          format_func=lambda i: object_labels[i])
